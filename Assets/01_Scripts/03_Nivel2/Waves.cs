...
public class Waves : MonoBehaviour
{
    ...
    public float GetHeight(Vector3 position)
    {
        ...
        p4.x = Mathf.Clamp(p4.z, 0, dimension);
        ...
        for (int x = 0; x <= dimension; x++)
        {
            for (int z = 0; z <= dimension; z++)
            {
                ...
                for (int o = 0; o < Octaves.Length; o++)
                {
                    ...
                    {
                        var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x) /
                            dimension, (z * Octaves[o].scale.y) / dimension) * Mathf.PI * 2f;
                        y += Mathf.Cos(perl + Octaves[o].speed.magnitude * Time.time) * Octaves[o].height;
                    }
                    else
                    {
                        var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x + Time.time * Octaves[o].speed.x) /
                            dimension, (z * Octaves[o].scale.y + Time.time * Octaves[o].speed.y) / dimension) - 0.5f;
